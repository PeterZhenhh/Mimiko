...
export const tracks = async ({
    ...
    console.log(`Fetching tracks for ${jFullCode} from jasmr...`);
    const url = `${getRemoteDomain()}/api/v1/videos?code=${jFullCode}`;
    let data: ApiVideo;
    try {
        console.log(url);
        ...
        const resp = await fetch(url, {
            headers: {
                ...
                referer: `https://www.jasmr.net/watch/${jFullCode}`,
            },
        });
        data = await resp.json();
    } catch (error) {
        console.error(
            `Error fetching tracks for ${jFullCode} from jasmr:`,
            error,
        );
    ...
    const ret: BaseTrackFile[] = [
        {
            type: "audio",
            fileName: `${data?.title?.chinese || data?.title?.japanese || data?.title?.english || `${jFullCode}`}_jasmr`,
